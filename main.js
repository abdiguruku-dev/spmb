...
if(muridForm) {
    muridForm.addEventListener('submit', async (e) => {
        ...
        payload.append('petugas_input', petugasName); payload.append('nama_lengkap', document.getElementById('nama_lengkap') ? document.getElementById('nama_lengkap').value : ""); payload.append('jenis_kelamin', document.getElementById('jenis_kelamin') ? document.getElementById('jenis_kelamin').value : ""); payload.append('nisn', document.getElementById('nisn') ? document.getElementById('nisn').value : ""); payload.append('nik', document.getElementById('nik') ? document.getElementById('nik').value : ""); payload.append('tempat_lahir', document.getElementById('tempat_lahir') ? document.getElementById('tempat_lahir').value : ""); payload.append('tanggal_lahir', document.getElementById('tanggal_lahir') ? document.getElementById('tanggal_lahir').value : ""); payload.append('agama', document.getElementById('agama') ? document.getElementById('agama').value : ""); payload.append('no_hp_murid', document.getElementById('no_hp_murid') ? document.getElementById('no_hp_murid').value : ""); 
        let dusun = document.getElementById('alamat_dusun') ? document.getElementById('alamat_dusun').value : ""; let rt = document.getElementById('alamat_rt') ? document.getElementById('alamat_rt').value : ""; let rw = document.getElementById('alamat_rw') ? document.getElementById('alamat_rw').value : "";
        payload.append('alamat_lengkap', `Dusun ${dusun}, RT ${rt}/RW ${rw}`); 
        payload.append('desa_kelurahan', document.getElementById('desa_kelurahan') ? document.getElementById('desa_kelurahan').value : ""); payload.append('kecamatan', document.getElementById('kecamatan') ? document.getElementById('kecamatan').value : ""); payload.append('jenis_tinggal', document.getElementById('jenis_tinggal') ? document.getElementById('jenis_tinggal').value : ""); payload.append('nama_wali', document.getElementById('nama_wali') ? document.getElementById('nama_wali').value : ""); payload.append('hp_wali', document.getElementById('hp_wali') ? document.getElementById('hp_wali').value : ""); payload.append('alamat_wali', document.getElementById('alamat_wali') ? document.getElementById('alamat_wali').value : ""); payload.append('no_hp_ortu', document.getElementById('no_hp_ortu') ? document.getElementById('no_hp_ortu').value : ""); payload.append('nama_ayah', document.getElementById('nama_ayah') ? document.getElementById('nama_ayah').value : ""); payload.append('pekerjaan_ayah', document.getElementById('pekerjaan_ayah') ? document.getElementById('pekerjaan_ayah').value : ""); payload.append('status_ayah', document.getElementById('status_ayah') ? document.getElementById('status_ayah').value : ""); payload.append('nama_ibu', document.getElementById('nama_ibu') ? document.getElementById('nama_ibu').value : ""); payload.append('pekerjaan_ibu', document.getElementById('pekerjaan_ibu') ? document.getElementById('pekerjaan_ibu').value : ""); payload.append('status_ibu', document.getElementById('status_ibu') ? document.getElementById('status_ibu').value : ""); payload.append('asal_sekolah', document.getElementById('asal_sekolah') ? document.getElementById('asal_sekolah').value : ""); payload.append('status_bantuan', document.getElementById('status_bantuan') ? document.getElementById('status_bantuan').value : ""); payload.append('entitas_pembawa', entitasTerpilih.join(", "));
            ...
            if(result.status === 'success') {
                if(typeof Swal !== 'undefined') {
                    Swal.fire({
                        ...
                        confirmButtonText: editModeId ? 'Tutup' : 'Input Baru',
...
async function deleteData(id) {
    ...
    const result = await Swal.fire({ title: 'Hapus Permanen?', text: `Anda yakin ingin menghapus data ${id} secara permanen? Data yang dihapus tidak bisa dikembalikan.`, icon: 'warning', showCancelButton: true, confirmButtonColor: '#ef4444', cancelButtonColor: '#64748b', confirmButtonText: 'Ya, Hapus!' });
    ...
    try {
        ...
        const res = await fetch(SCRIPT_URL.trim(), { method: 'POST', body: payload }); const r = await res.json();
        if(r.status === 'success') { Swal.fire('Terhapus!', 'Data berhasil dihapus secara permanen.', 'success'); loadMasterData(false); } 
        else { Swal.fire('Gagal', r.message, 'error'); }
    } catch(e) { Swal.fire('Error', 'Koneksi terputus', 'error'); }
}

// ========================================================
// AMAN: FUNGSI TOMBOL LAPOR (VISIT & RIWAYAT)
// ========================================================
...
// ========================================================
// SISANYA: DASHBOARD, SETTINGS, EXCEL, USER ADMIN
// ========================================================
...
function doDrillDownUtama(field, value) { 
    ...
    if(sInp) { sInp.value = `filter:${field}=${value}`; filterTabel(`filter:${field}=${value}`); }
}
...
const fManualVisit = document.getElementById('formManualVisit');
if(fManualVisit) {
    fManualVisit.addEventListener('submit', async (e) => {
        e.preventDefault(); 
        ...
        try {
            ...
            if(r.status === 'success') { 
                ...
                fManualVisit.reset(); if (typeof $ !== 'undefined') $('select').trigger('change'); loadVisitData(false); 
...
const btnImpExcel = document.getElementById('btnImportExcel');
if(btnImpExcel) {
    btnImpExcel.addEventListener('click', () => {
