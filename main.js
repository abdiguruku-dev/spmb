...
// Event Handler: Buka Lapor Visit
// MEMPERBAIKI MASALAH NAMA DENGAN TANDA KUTIP MENGGUNAKAN EVENT DELEGATION DI JQUERY
if (typeof $ !== 'undefined') {
...
if(document.getElementById('formManualVisit')) {
    document.getElementById('formManualVisit').addEventListener('submit', async (e) => {
        ...
        try {
            ...
            const r = await res.json();
            if(r.status === 'success') { 
                Swal.fire({ icon: 'success', title: 'Tersimpan!', text: 'Target Visit berhasil ditambah!' });
    ...
    document.getElementById('btnImportExcel').addEventListener('click', () => {
        ...
        reader.onload = async (e) => {
            try {
                ...
                Swal.fire({ icon: 'error', title: 'Format Salah', text: 'Gagal memproses file Excel. Pastikan format benar.' });
            }
        };
...
function editStok(nama, awal, masuk) {
    ...
    document.getElementById('stok_awal').value = awal;
    document.getElementById('stok_masuk').value = masuk;
    document.getElementById('modalEditStok').classList.remove('spa-hidden');
}
...
if(document.getElementById('formEditStokLogic')) {
    document.getElementById('formEditStokLogic').addEventListener('submit', async(e) => {
        ...
        payload.append('nama_barang', document.getElementById('stok_nama').value); 
        payload.append('stok_awal', document.getElementById('stok_awal').value); 
        payload.append('barang_masuk', document.getElementById('stok_masuk').value); 
        ...
        try {
            ...
            if(r.status === 'success') { 
                Swal.fire({ icon: 'success', title: 'Berhasil!', text: 'Stok berhasil diupdate!' });
                document.getElementById('modalEditStok').classList.add('spa-hidden'); 
                loadStokData(); 
            } else {
                Swal.fire({ icon: 'error', title: 'Gagal', text: r.message });
            }
...
function doDrillDownUtama(field, value) { 
    ...
    switchView('tabel'); 
    document.getElementById('searchInput').value = `filter:${field}=${value}`; 
...
function editData(id) {
    ...
    try { 
        ...
        console.error("Format alamat gagal di-parse", e);
    }
    ...
    if(data.tanggal_lahir) { 
        try { 
            document.getElementById('tanggal_lahir').value = new Date(data.tanggal_lahir).toISOString().split('T')[0]; 
        } catch(e){} 
    }
    ...
    document.getElementById('status_ayah').value = data.status_ayah || "Masih Hidup"; 
    document.getElementById('nama_ibu').value = data.nama_ibu || ""; 
    
    setSelect2Value('pekerjaan_ibu', data.pekerjaan_ibu);
    document.getElementById('status_ibu').value = data.status_ibu || "Masih Hidup"; 
    
    setSelect2Value('asal_sekolah', data.asal_sekolah);
    
    document.getElementById('status_bantuan').value = data.status_bantuan || "Tidak Ada";
    
    const arrEntitas = (data.entitas_pembawa || "").split(", "); 
    document.querySelectorAll('input[name="chk_entitas"]').forEach(chk => { 
        chk.checked = arrEntitas.includes(chk.value); 
    });
    
    ['atas_osis', 'bawah_osis', 'atas_or', 'bawah_or'].forEach(u => {
        let el = document.getElementById(`ukur_${u}`);
        if(el) el.value = data[`ukuran_${u}`] || '-';
    }); 
    
    ['formulir', 'skl', 'nilai', 'tka', 'foto', 'ijazah', 'kk', 'akte', 'bantuan'].forEach(b => {
        let el = document.getElementById(`berkas_${b}`);
        if(el) el.value = data[`berkas_${b}`] || 'Belum Ada';
    });
...
if(document.getElementById('formLogistik')) {
    document.getElementById('formLogistik').addEventListener('submit', async (e) => {
        ...
        try {
            ...
            const r = await res.json();
            if(r.status === 'success') { 
                Swal.fire({ icon: 'success', title: 'Berhasil!', text: 'Penyerahan berhasil dicatat!' });
