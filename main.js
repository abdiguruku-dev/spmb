...
function switchView(viewName) {
    ...
    views.forEach(v => { if(document.getElementById(v)) document.getElementById(v).classList.add('spa-hidden'); });
    ...
    navs.forEach(n => { if(document.getElementById(n)) document.getElementById(n).classList.remove('bg-blue-50','text-blue-700'); });
...
function setSelect2Value(id, value) {
    if (!value || value === "-" || value === "") { $('#' + id).val('').trigger('change'); return; }
    if ($('#' + id).find("option[value='" + value + "']").length) { $('#' + id).val(value).trigger('change'); } 
    else { var newOption = new Option(value, value, true, true); $('#' + id).append(newOption).trigger('change'); } 
}

function bukaFormBaru() {
    editModeId = null; document.getElementById('muridForm').reset(); 
...
function toggleWali(status) {
    ...
    else { formWali.classList.add('hidden'); document.getElementById('nama_wali').value = ''; document.getElementById('hp_wali').value = ''; document.getElementById('alamat_wali').value = ''; }
}

function applySettings(settings) {
    if(settings.app_name) { 
        document.getElementById('loginAppName').innerText = settings.app_name; document.getElementById('sidebarAppName').innerText = settings.app_name; document.getElementById('appDocumentTitle').innerText = settings.app_name; 
    }
    if(settings.logo_url) {
        const lgLogin = document.getElementById('loginLogoImage'); const lgSidebar = document.getElementById('sidebarLogoImage');
        lgLogin.src = settings.logo_url; lgLogin.classList.remove('hidden'); document.getElementById('loginLogoIcon').classList.add('hidden');
        lgSidebar.src = settings.logo_url; lgSidebar.classList.remove('hidden'); document.getElementById('sidebarLogoIcon').classList.add('hidden');
    }
    if(settings.favicon_url) document.getElementById('appFavicon').href = settings.favicon_url; 
...
document.addEventListener('DOMContentLoaded', async () => { 
    ...
    try { 
        const s = await fetch(SCRIPT_URL.trim() + "?action=get_settings"); const textRes = await s.text(); const sRes = JSON.parse(textRes); 
        if(sRes.status === 'success') applySettings(sRes.data); 
    } catch(e){}
    
    const saved = sessionStorage.getItem('spmb_session'); 
    if(saved) { tampilkanApp(JSON.parse(saved)); startAutoSync(); }
    
    const chkContainer = document.getElementById('checklistAtributContainer');
    if(chkContainer) { 
        listAtributLogistik.forEach(attr => { chkContainer.innerHTML += `<label class="flex items-center space-x-2 text-sm bg-slate-50 p-2 rounded border cursor-pointer hover:bg-slate-100"><input type="checkbox" name="chk_logistik_attr" value="${attr}" class="rounded text-blue-600 focus:ring-blue-500"><span>${attr}</span></label>`; }); 
    }
});
...
document.getElementById('formProfile').addEventListener('submit', async (e) => {
    ...
    payload.append('new_password', document.getElementById('prof_new_pass').value);
    payload.append('foto_profil', document.getElementById('prof_foto_url').value);
...
document.getElementById('loginForm').addEventListener('submit', async (e) => {
    ...
    try {
        const fd = new URLSearchParams(); fd.append('action', 'login'); fd.append('username', document.getElementById('username').value.trim()); fd.append('password', document.getElementById('password').value.trim());
        ...
        if (result.status === 'success') { 
            ...
            Swal.close(); tampilkanApp(result.data); startAutoSync(); 
        } else { Swal.fire('Gagal', result.message, 'error'); } 
    } catch (err) { Swal.fire('Error', 'Koneksi Gagal: ' + err.message, 'error'); }
});

document.getElementById('btnLogout').addEventListener('click', () => { 
    sessionStorage.removeItem('spmb_session'); sessionStorage.removeItem('lastActiveTime'); if(window.syncInterval) clearInterval(window.syncInterval); location.reload(); 
});
...
async function tampilkanApp(userData) {
    ...
    ['panelPendaftar', 'panelSDDesa', 'berkasPanel', 'logistikPanel', 'visitStatsPanel', 'adminUsersStatsPanel'].forEach(id => { if(document.getElementById(id)) document.getElementById(id).classList.add('hidden'); });
    ['panelPendaftar', 'panelSDDesa'].forEach(id => { if(document.getElementById(id)) document.getElementById(id).classList.remove('hidden'); });
    ...
    if (roleStr === "administrator") {
        ...
        allLabels.forEach(l => document.getElementById(l).classList.remove('hidden')); document.getElementById('btnTambahPendaftarUtama').classList.remove('hidden'); 
        ['adminUsersStatsPanel', 'berkasPanel', 'logistikPanel', 'visitStatsPanel'].forEach(id => { if(document.getElementById(id)) document.getElementById(id).classList.remove('hidden'); });
        loadUsersData(); 
    } 
    else if (roleStr === "kepala sekolah" || roleStr === "ketua panitia") {
        ['navTabel', 'navStokLogistik', 'navLogistik'].forEach(n => document.getElementById(n).classList.remove('hidden')); document.getElementById('labelMenuLogistik').classList.remove('hidden');
        ['berkasPanel', 'logistikPanel'].forEach(id => { if(document.getElementById(id)) document.getElementById(id).classList.remove('hidden'); });
        document.getElementById('btnTambahPendaftarUtama').classList.add('hidden'); 
    } 
    else if (roleStr === "admin input") {
        document.getElementById('navTabel').classList.remove('hidden'); document.getElementById('navKelolaVisit').classList.remove('hidden'); document.getElementById('btnTambahPendaftarUtama').classList.remove('hidden'); document.getElementById('berkasPanel').classList.remove('hidden');
    } 
    else if (roleStr === "petugas seragam" || roleStr === "logistik") {
        document.getElementById('navTabel').classList.remove('hidden'); document.getElementById('navStokLogistik').classList.remove('hidden'); document.getElementById('navLogistik').classList.remove('hidden'); document.getElementById('labelMenuLogistik').classList.remove('hidden'); document.getElementById('logistikPanel').classList.remove('hidden'); document.getElementById('btnTambahPendaftarUtama').classList.add('hidden');
    } 
    else if (roleStr === "guru") {
        document.getElementById('navTabel').classList.remove('hidden'); document.getElementById('btnTambahPendaftarUtama').classList.add('hidden');
    }
...
if(document.getElementById('formSettings')) {
    document.getElementById('formSettings').addEventListener('submit', async(e) => {
        e.preventDefault(); const btn = document.getElementById('btnSaveSettings'); btn.disabled = true; btn.innerText="Menyimpan...";
        const payload = new URLSearchParams(); payload.append('action', 'save_settings'); payload.append('app_name', document.getElementById('set_app_name').value); payload.append('app_desc', document.getElementById('set_app_desc').value); payload.append('logo_url', document.getElementById('set_logo_url').value); payload.append('favicon_url', document.getElementById('set_favicon_url').value); payload.append('sekolah_nama', document.getElementById('set_sekolah_nama').value); payload.append('sekolah_akreditasi', document.getElementById('set_sekolah_akreditasi').value); payload.append('sekolah_alamat', document.getElementById('set_sekolah_alamat').value); payload.append('sekolah_email', document.getElementById('set_sekolah_email').value); payload.append('sekolah_website', document.getElementById('set_sekolah_website').value);
        try {
            const res = await fetch(SCRIPT_URL.trim(), {method:'POST', body:payload}); const r = await res.json();
            if(r.status === 'success') { 
                Swal.fire('Berhasil!', 'Pengaturan tersimpan!', 'success');
                applySettings({ app_name: document.getElementById('set_app_name').value, app_desc: document.getElementById('set_app_desc').value, logo_url: document.getElementById('set_logo_url').value, favicon_url: document.getElementById('set_favicon_url').value, sekolah_nama: document.getElementById('set_sekolah_nama').value, sekolah_akreditasi: document.getElementById('set_sekolah_akreditasi').value, sekolah_alamat: document.getElementById('set_sekolah_alamat').value, sekolah_email: document.getElementById('set_sekolah_email').value, sekolah_website: document.getElementById('set_sekolah_website').value }); 
            }
        } catch(err) { Swal.fire('Error', 'Gagal terhubung ke Server', 'error'); } finally { btn.disabled = false; btn.innerHTML = `<i class="ph ph-floppy-disk align-middle mr-1"></i> Simpan Profil & Identitas`; }
    });
}
...
async function fetchReferences() {
    try {
        ...
        if(result.status === "success") {
            ...
            if(elCheck) { elCheck.innerHTML = ''; const combinedEntitas = [...(result.data.entitas || []), ...(result.data.individu || [])]; combinedEntitas.forEach(item => { elCheck.innerHTML += `<label class="flex items-center space-x-2 text-sm bg-slate-50 p-2 rounded border cursor-pointer hover:bg-slate-100 transition-colors"><input type="checkbox" name="chk_entitas" value="${item}" class="rounded text-blue-600 focus:ring-blue-500"><span>${item}</span></label>`; }); }
...
async function loadMasterData(isBackground = false) {
    try {
        ...
        if (result.status === "success") { 
            ...
            let qUtama = document.getElementById('searchInput') ? document.getElementById('searchInput').value.trim() : ""; if (qUtama === "") { try { renderTabel(masterData); } catch(e){} }
            let qLog = document.getElementById('searchLogistik') ? document.getElementById('searchLogistik').value.trim() : ""; if (qLog === "") { try { renderLogistik(masterData); } catch(e){} }
        }
    } catch (error) {}
}

async function loadStokData() {
    try { const response = await fetch(SCRIPT_URL.trim() + "?action=get_stok"); const result = await response.json(); if (result.status === "success") { stockData = result.data; } } catch (error) {}
}
...
async function loadVisitData(isBackground = false) {
    ...
    try {
        ...
        if (result.status === "success") { 
            ...
            let qV = document.getElementById('searchVisit') ? document.getElementById('searchVisit').value.trim() : ""; let qR = document.getElementById('searchRiwayatVisit') ? document.getElementById('searchRiwayatVisit').value.trim() : ""; let qH = document.getElementById('searchHasilVisit') ? document.getElementById('searchHasilVisit').value.trim() : "";
...
function editData(id) {
    const data = masterData.find(m => m.id_pendaftaran === id); if(!data) return; editModeId = id; 
    ...
    document.getElementById('nama_lengkap').value = data.nama_lengkap; document.getElementById('jenis_kelamin').value = data.jenis_kelamin; document.getElementById('nisn').value = data.nisn; document.getElementById('nik').value = data.nik; document.getElementById('tempat_lahir').value = data.tempat_lahir; 
    ...
    document.getElementById('agama').value = data.agama; document.getElementById('no_hp_murid').value = data.no_hp_murid; document.getElementById('alamat_dusun').value = dusun; document.getElementById('alamat_rt').value = rt; document.getElementById('alamat_rw').value = rw; 
    ...
    document.getElementById('nama_wali').value = data.nama_wali; document.getElementById('hp_wali').value = data.hp_wali; document.getElementById('alamat_wali').value = data.alamat_wali; document.getElementById('no_hp_ortu').value = data.no_hp_ortu; document.getElementById('nama_ayah').value = data.nama_ayah; 
    setSelect2Value('pekerjaan_ayah', data.pekerjaan_ayah); document.getElementById('status_ayah').value = data.status_ayah; document.getElementById('nama_ibu').value = data.nama_ibu; setSelect2Value('pekerjaan_ibu', data.pekerjaan_ibu); document.getElementById('status_ibu').value = data.status_ibu; setSelect2Value('asal_sekolah', data.asal_sekolah);
    document.getElementById('status_bantuan').value = data.status_bantuan; const arrEntitas = (data.entitas_pembawa || "").split(", "); document.querySelectorAll('input[name="chk_entitas"]').forEach(chk => { chk.checked = arrEntitas.includes(chk.value); });
    ['atas_osis', 'bawah_osis', 'atas_or', 'bawah_or'].forEach(u => document.getElementById(`ukur_${u}`).value = data[`ukuran_${u}`] || '-'); ['formulir', 'skl', 'nilai', 'tka', 'foto', 'ijazah', 'kk', 'akte', 'bantuan'].forEach(b => document.getElementById(`berkas_${b}`).value = data[`berkas_${b}`] || 'Belum Ada');
    switchView('form');
    if (typeof $ !== 'undefined') $('select').trigger('change');
}
...
function bukaModalLogistik(id) {
    ...
    document.getElementById('stat_osis').disabled = !allowEdit; document.getElementById('stat_or').disabled = !allowEdit; document.getElementById('diserahkan_kepada').disabled = !allowEdit; document.querySelectorAll('input[name="chk_logistik_attr"]').forEach(chk => { chk.disabled = !allowEdit; });
    const btnSubmit = document.getElementById('btnSubmitLogistik'); if(!allowEdit) { btnSubmit.classList.add('hidden'); } else { btnSubmit.classList.remove('hidden'); }
    document.getElementById('logistikModal').classList.remove('spa-hidden'); initSelect2Global();
}
    ...
    try {
        ...
        if(result.status === 'success') {
            Swal.fire({
                ...
                html: `Data telah masuk ke database.<br><br><b>ID: ${editModeId ? editModeId : result.data.id_pendaftaran}</b>`,
                icon: 'success',
                ...
                if (resSwal.isConfirmed) {
                    if (editModeId) { editModeId = null; document.getElementById('muridForm').reset(); switchView('tabel'); } 
                    else { bukaFormBaru(); }
                } else {
                    editModeId = null; document.getElementById('muridForm').reset(); switchView('tabel');
                }
            });
