...
if(btnCloseSidebar) btnCloseSidebar.addEventListener('click', closeMenu); 
if(sidebarOverlay) sidebarOverlay.addEventListener('click', closeMenu);

// ==========================================
// NOTIFIKASI TOAST SWEETALERT2 (MODERN)
// ==========================================
...
function resetIdleTimer() { 
    ...
    if(!loginView || !loginView.classList.contains('spa-hidden')) return; 
    sessionStorage.setItem('lastActiveTime', Date.now()); 
}

function checkIdleTime() {
    const loginView = document.getElementById('loginView');
    if(loginView && loginView.classList.contains('spa-hidden')) {
        ...
        if (lastActive && (Date.now() - parseInt(lastActive) >= IDLE_TIMEOUT)) {
            ...
            if (typeof Swal !== 'undefined') {
                Swal.fire({ icon: 'warning', title: 'Sesi Berakhir', text: 'Sesi Anda telah berakhir karena tidak ada aktivitas selama 15 menit.' });
            }
        }
    }
}
...
function switchView(viewName) {
    ...
    views.forEach(v => { 
        ...
        if(el) el.classList.add('spa-hidden'); 
    });
...
function bukaFormBaru() {
    ...
    if(fTitle) fTitle.innerHTML = `<i class="ph-fill ph-file-plus text-blue-600"></i> Input Pendaftaran Murid Baru`; 
    
    const btnSubmit = document.getElementById('btnSubmitMurid');
    if(btnSubmit) btnSubmit.innerHTML = `<i class="ph ph-floppy-disk text-xl"></i> Simpan Data Pendaftar`; 
...
function toggleWali(status) {
    ...
    if (status === "Bersama Wali/Kerabat") { 
        ...
        if(document.getElementById('nama_wali')) document.getElementById('nama_wali').value = ''; 
        if(document.getElementById('hp_wali')) document.getElementById('hp_wali').value = ''; 
        if(document.getElementById('alamat_wali')) document.getElementById('alamat_wali').value = ''; 
    }
}
...
function applySettings(settings) {
    ...
    if(settings.favicon_url) { 
        if(document.getElementById('appFavicon')) document.getElementById('appFavicon').href = settings.favicon_url; 
    }
...
// Event Handler: Buka Modal Profil
    ...
    formProfile.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if(r.status === 'success') {
                ...
                if(typeof Swal !== 'undefined') Swal.fire('Gagal', r.message, 'error');
            }
        } catch(err) {
...
if(loginForm) {
    loginForm.addEventListener('submit', async (e) => {
        ...
        if(!usernameInput || !passwordInput) {
            if(typeof Swal !== 'undefined') Swal.fire({ icon: 'warning', title: 'Perhatian', text: 'Username dan Password wajib diisi!' });
            return;
        }
        ...
        try {
            ...
            if (result.status === 'success') { 
                ...
                if(typeof Swal !== 'undefined') Swal.fire({ icon: 'error', title: 'Login Gagal', text: result.message });
            } 
        } catch (err) { 
            if(typeof Swal !== 'undefined') Swal.fire({ icon: 'error', title: 'Koneksi Bermasalah', text: err.message });
        }
    });
}
...
async function tampilkanApp(userData) {
    const loginView = document.getElementById('loginView');
    if(loginView) loginView.classList.add('spa-hidden'); 
    
    const appView = document.getElementById('appView');
    if(appView) appView.classList.remove('spa-hidden');
    
    const userNameDisplay = document.getElementById('userNameDisplay');
    if(userNameDisplay) userNameDisplay.innerText = userData.nama_lengkap; 
    
    const userRoleBadge = document.getElementById('userRoleBadge');
    if(userRoleBadge) userRoleBadge.innerText = userData.role;
    
    let profIcon = document.getElementById('sidebarAvatarIcon');
    let profImg = document.getElementById('sidebarAvatar');
    if(userData.foto_profil && userData.foto_profil.trim() !== "") {
        if(profIcon) profIcon.classList.add('hidden');
        if(profImg) { 
            profImg.src = userData.foto_profil; 
    ...
    if (roleStr === "administrator") {
        ...
        const btnTambah = document.getElementById('btnTambahPendaftarUtama');
        ...
        if(document.getElementById('navTabel')) document.getElementById('navTabel').classList.remove('hidden'); 
        if(document.getElementById('navKelolaVisit')) document.getElementById('navKelolaVisit').classList.remove('hidden'); 
        if(document.getElementById('btnTambahPendaftarUtama')) document.getElementById('btnTambahPendaftarUtama').classList.remove('hidden'); 
        if(document.getElementById('berkasPanel')) document.getElementById('berkasPanel').classList.remove('hidden');
    } 
    else if (roleStr === "petugas seragam" || roleStr === "logistik") {
        if(document.getElementById('navTabel')) document.getElementById('navTabel').classList.remove('hidden'); 
        if(document.getElementById('navStokLogistik')) document.getElementById('navStokLogistik').classList.remove('hidden'); 
        if(document.getElementById('navLogistik')) document.getElementById('navLogistik').classList.remove('hidden'); 
        if(document.getElementById('labelMenuLogistik')) document.getElementById('labelMenuLogistik').classList.remove('hidden'); 
        if(document.getElementById('logistikPanel')) document.getElementById('logistikPanel').classList.remove('hidden'); 
        if(document.getElementById('btnTambahPendaftarUtama')) document.getElementById('btnTambahPendaftarUtama').classList.add('hidden');
    } 
...
if(formSettings) {
    formSettings.addEventListener('submit', async(e) => {
        ...
        payload.append('app_name', document.getElementById('set_app_name') ? document.getElementById('set_app_name').value : ""); 
        payload.append('app_desc', document.getElementById('set_app_desc') ? document.getElementById('set_app_desc').value : "");
        payload.append('logo_url', document.getElementById('set_logo_url') ? document.getElementById('set_logo_url').value : "");
        payload.append('favicon_url', document.getElementById('set_favicon_url') ? document.getElementById('set_favicon_url').value : "");
        payload.append('sekolah_nama', document.getElementById('set_sekolah_nama') ? document.getElementById('set_sekolah_nama').value : "");
        payload.append('sekolah_akreditasi', document.getElementById('set_sekolah_akreditasi') ? document.getElementById('set_sekolah_akreditasi').value : "");
        payload.append('sekolah_alamat', document.getElementById('set_sekolah_alamat') ? document.getElementById('set_sekolah_alamat').value : "");
        payload.append('sekolah_email', document.getElementById('set_sekolah_email') ? document.getElementById('set_sekolah_email').value : "");
        payload.append('sekolah_website', document.getElementById('set_sekolah_website') ? document.getElementById('set_sekolah_website').value : "");
        ...
        try {
            ...
            if(r.status === 'success') { 
                ...
                applySettings({
                    app_name: document.getElementById('set_app_name') ? document.getElementById('set_app_name').value : "", 
                    app_desc: document.getElementById('set_app_desc') ? document.getElementById('set_app_desc').value : "",
                    logo_url: document.getElementById('set_logo_url') ? document.getElementById('set_logo_url').value : "",
                    favicon_url: document.getElementById('set_favicon_url') ? document.getElementById('set_favicon_url').value : "",
                    sekolah_nama: document.getElementById('set_sekolah_nama') ? document.getElementById('set_sekolah_nama').value : "",
                    sekolah_akreditasi: document.getElementById('set_sekolah_akreditasi') ? document.getElementById('set_sekolah_akreditasi').value : "",
                    sekolah_alamat: document.getElementById('set_sekolah_alamat') ? document.getElementById('set_sekolah_alamat').value : "",
                    sekolah_email: document.getElementById('set_sekolah_email') ? document.getElementById('set_sekolah_email').value : "",
                    sekolah_website: document.getElementById('set_sekolah_website') ? document.getElementById('set_sekolah_website').value : ""
                }); 
            } else {
                if(typeof Swal !== 'undefined') Swal.fire({ icon: 'error', title: 'Gagal', text: r.message });
            }
        } catch(err) { 
            if(typeof Swal !== 'undefined') Swal.fire({ icon: 'error', title: 'Error', text: 'Gagal terhubung ke Server' });
        }
    });
}
...
async function loadMasterData(isBackground = false) {
    try {
        ...
        if (result.status === "success") { 
            ...
            let qUtama = searchInp ? searchInp.value.trim() : "";
            if (qUtama === "") {
                try { renderTabel(masterData); } catch(e){}
            }

            let logInp = document.getElementById('searchLogistik');
            let qLog = logInp ? logInp.value.trim() : "";
            if (qLog === "") {
...
async function loadVisitData(isBackground = false) {
    ...
    try {
        ...
        if (result.status === "success") { 
            ...
            let searchVis = document.getElementById('searchVisit');
            let qV = searchVis ? searchVis.value.trim() : "";
            
            let searchRiw = document.getElementById('searchRiwayatVisit');
            let qR = searchRiw ? searchRiw.value.trim() : "";
            
            let searchHas = document.getElementById('searchHasilVisit');
            let qH = searchHas ? searchHas.value.trim() : "";
...
if(formLaporVisit) {
    formLaporVisit.addEventListener('submit', async (e) => {
        ...
        const timAnak = formLaporVisit.dataset.tim;
        ...
        try {
            ...
            if(r.status === 'success') { 
                ...
                let modLap = document.getElementById('laporVisitModal');
                if(modLap) modLap.classList.add('spa-hidden'); 
            ...
            if(typeof Swal !== 'undefined') Swal.fire({ icon: 'error', title: 'Error Jaringan', text: 'Gagal menghubungi server.' }); 
        }
    });
}
...
if(formManualVisit) {
    formManualVisit.addEventListener('submit', async (e) => {
        ...
        payload.append('nama_lengkap', document.getElementById('v_nama') ? document.getElementById('v_nama').value : ""); 
        payload.append('lp', document.getElementById('v_lp') ? document.getElementById('v_lp').value : "");
        payload.append('asal_sekolah', document.getElementById('v_sd') ? document.getElementById('v_sd').value : ""); 
        payload.append('tim_alokasi', document.getElementById('v_tim') ? document.getElementById('v_tim').value : "");
        payload.append('nama_ayah', document.getElementById('v_ayah') ? document.getElementById('v_ayah').value : ""); 
        payload.append('nama_ibu', document.getElementById('v_ibu') ? document.getElementById('v_ibu').value : "");
        payload.append('alamat', document.getElementById('v_alamat') ? document.getElementById('v_alamat').value : "");
        payload.append('tanggal_mulai', document.getElementById('v_mulai') ? document.getElementById('v_mulai').value : ""); 
        payload.append('batas_waktu', document.getElementById('v_batas') ? document.getElementById('v_batas').value : ""); 
        payload.append('desa_kelurahan', document.getElementById('v_desa') ? document.getElementById('v_desa').value : ""); 
        payload.append('kecamatan', document.getElementById('v_kecamatan') ? document.getElementById('v_kecamatan').value : "");
...
if(btnImportExcel) {
    btnImportExcel.addEventListener('click', () => {
        ...
        if(!fileInput || !fileInput.files.length) {
            if(typeof Swal !== 'undefined') Swal.fire({ icon: 'warning', title: 'Perhatian', text: 'Pilih file Excel terlebih dahulu!' });
            return;
        }
        ...
        reader.onload = async (e) => {
            try {
                ...
                if(typeof Swal !== 'undefined') Swal.fire({ icon: 'error', title: 'Format Salah', text: 'Gagal memproses file Excel. Pastikan format benar.' });
            }
        };
...
if(muridForm) {
    muridForm.addEventListener('submit', async (e) => {
        ...
        let entitasTerpilih = []; 
        document.querySelectorAll('input[name="chk_entitas"]:checked').forEach(chk => entitasTerpilih.push(chk.value)); 
        
        const payload = new URLSearchParams(); 
        if (editModeId) { 
            payload.append('action', 'update_murid'); 
            payload.append('id_pendaftaran', editModeId); 
        } else { 
            payload.append('action', 'insert_murid'); 
        }
        
        let sess = sessionStorage.getItem('spmb_session');
        let petugasName = sess ? JSON.parse(sess).nama_lengkap : "Unknown";
        ...
        ['atas_osis', 'bawah_osis', 'atas_or', 'bawah_or'].forEach(u => {
            let val = document.getElementById(`ukur_${u}`) ? document.getElementById(`ukur_${u}`).value : "-";
            payload.append(`ukuran_${u}`, val);
        }); 
        
        ['formulir', 'skl', 'nilai', 'tka', 'foto', 'ijazah', 'kk', 'akte', 'bantuan'].forEach(b => {
            let val = document.getElementById(`berkas_${b}`) ? document.getElementById(`berkas_${b}`).value : "Belum Ada";
            payload.append(`berkas_${b}`, val);
        });
        
        try {
            const response = await fetch(SCRIPT_URL.trim(), { method: 'POST', body: payload }); 
            const result = await response.json();
            ...
            if(result.status === 'success') {
                if(typeof Swal !== 'undefined') {
                    Swal.fire({
                        ...
                        confirmButtonText: editModeId ? 'Tutup Form' : 'Input Lagi',
                        cancelButtonText: 'Ke Tabel Data'
                    }).then((resSwal) => {
                        if (resSwal.isConfirmed) {
                            if (editModeId) { editModeId = null; muridForm.reset(); switchView('tabel'); } 
                            else { bukaFormBaru(); }
                        } else {
                            editModeId = null; muridForm.reset(); switchView('tabel');
                        }
                    });
                }
            ...
            if(typeof Swal !== 'undefined') Swal.fire('Error', 'Koneksi terputus: ' + err.message, 'error'); 
        } finally { 
...
function showDetailTable(title, globalKey) {
    let modTitle = document.getElementById('detailTableTitle');
    if(modTitle) modTitle.innerHTML = `<i class="ph-fill ph-list-magnifying-glass"></i> ${title}`;
    ...
    let modDet = document.getElementById('modalDetailTable');
    if(modDet) modDet.classList.remove('spa-hidden');
}
...
function prosesDashboard(data) {
    ...
    if(tbodySD) { 
        ...
        Object.keys(rSD).sort((a,b) => rSD[b].T - rSD[a].T).forEach(k => { 
            tbodySD.innerHTML += `<tr class="clickable-row border-b" onclick="doDrillDownUtama('asal_sekolah', '${k.replace(/'/g, "\\'")}')"><td class="p-3">${k}</td><td class="p-3 text-center">${rSD[k].L}</td><td class="p-3 text-center">${rSD[k].P}</td><td class="p-3 text-center font-bold text-blue-600">${rSD[k].T}</td></tr>`; 
        });
    }
    ...
    if(tbodyDesa) { 
        ...
        Object.keys(rDesa).sort((a,b) => rDesa[b].T - rDesa[a].T).forEach(k => { 
            tbodyDesa.innerHTML += `<tr class="clickable-row border-b" onclick="doDrillDownUtama('desa_kelurahan', '${k.replace(/'/g, "\\'")}')"><td class="p-3">${k}</td><td class="p-3 text-center">${rDesa[k].L}</td><td class="p-3 text-center">${rDesa[k].P}</td><td class="p-3 text-center font-bold text-emerald-600">${rDesa[k].T}</td></tr>`; 
        });
    }
        ...
        berkasKunci.forEach(b => {
            ...
            if (jlmBelum > 0) { 
                card.onclick = () => { 
                    ...
                    if(sInput) sInput.value = `missing:${b.key}`; 
                    filterTabel(`missing:${b.key}`); 
                }; 
            } 
...
function renderStokLogistik() {
    ...
    baseItemsLogistik.forEach((item, index) => {
        ...
        let keluar = tallyKeluar[item] || 0; let butuh = (tallyKebutuhan[item] || 0) - keluar; 
        if (butuh < 0) butuh = 0;
...
function doDrillDownUtama(field, value) { 
    ...
    if(sInp) {
        sInp.value = `filter:${field}=${value}`; 
        filterTabel(`filter:${field}=${value}`); 
    }
}
...
function viewData(id) {
    ...
    let modView = document.getElementById('viewModal');
    if(modView) modView.classList.remove('spa-hidden');
}

function editData(id) {
    const data = masterData.find(m => m.id_pendaftaran === id); 
    if(!data) {
        if(typeof Swal !== 'undefined') Swal.fire('Error', 'Data tidak ditemukan.', 'error');
        return;
    }
    ...
    if(fTitle) fTitle.innerHTML = `<i class="ph-fill ph-pencil-simple text-amber-500"></i> Edit Data: ${id}`; 
    
    let btnSubmit = document.getElementById('btnSubmitMurid');
    if(btnSubmit) btnSubmit.innerHTML = `<i class="ph ph-check-circle text-xl"></i> Update Data Pendaftar`;
    ...
    } catch(e) { console.error("Parse alamat error:", e); }
    
    // SAFE ASSIGNMENT ALL FORM FIELDS
    ...
    if(data.tanggal_lahir && document.getElementById('tanggal_lahir')) { 
    ...
    if(data.jenis_tinggal && document.getElementById('jenis_tinggal')) { 
...
async function deleteData(id) {
    ...
    const result = await Swal.fire({
        title: 'Hapus Permanen?', text: `Anda yakin ingin menghapus data ${id} secara permanen?`, icon: 'warning', showCancelButton: true, confirmButtonColor: '#ef4444', cancelButtonColor: '#64748b', confirmButtonText: 'Ya, Hapus!'
    });
...
// Event Delegation untuk Tabel Visitasi
if (typeof $ !== 'undefined') {
    $(document).on('click', '.btn-lapor-visit', function(e) {
        e.preventDefault();
...
if(fLaporVisit) {
    fLaporVisit.addEventListener('submit', async (e) => {
        ...
        const user = JSON.parse(sessionStorage.getItem('spmb_session')); 
        const timAnak = fLaporVisit.dataset.tim;
        ...
        try {
            const res = await fetch(SCRIPT_URL.trim(), { method: 'POST', body: payload }); const r = await res.json();
            if(r.status === 'success') { 
                if(typeof Swal !== 'undefined') Swal.fire('Berhasil!', 'Laporan kunjungan dikirim!', 'success'); 
...
if(fAddUser) {
    fAddUser.addEventListener('submit', async(e) => {
        ...
        let uUser = document.getElementById('u_username') ? document.getElementById('u_username').value : "";
        let uPass = document.getElementById('u_password') ? document.getElementById('u_password').value : "";
        let uNama = document.getElementById('u_nama') ? document.getElementById('u_nama').value : "";
        let uRole = document.getElementById('u_role') ? document.getElementById('u_role').value : "";
        let uTim = document.getElementById('u_tim') ? document.getElementById('u_tim').value : "";
...
function editUser(username, password, nama, role, tim) {
    ...
    let mEu = document.getElementById('modalEditUser');
    if(mEu) mEu.classList.remove('spa-hidden');
    initSelect2Global();
}
...
if(fEditUser) {
    fEditUser.addEventListener('submit', async(e) => {
        ...
        let eOldUser = document.getElementById('edit_old_username') ? document.getElementById('edit_old_username').value : "";
        let eUser = document.getElementById('edit_u_username') ? document.getElementById('edit_u_username').value : "";
        let ePass = document.getElementById('edit_u_password') ? document.getElementById('edit_u_password').value : "";
        let eNama = document.getElementById('edit_u_nama') ? document.getElementById('edit_u_nama').value : "";
        let eRole = document.getElementById('edit_u_role') ? document.getElementById('edit_u_role').value : "";
        let eTim = document.getElementById('edit_u_tim') ? document.getElementById('edit_u_tim').value : "";
        ...
        try {
            ...
            if(r.status === 'success') { 
                ...
                let mEu = document.getElementById('modalEditUser');
                if(mEu) mEu.classList.add('spa-hidden'); 
                loadUsersData(); 
            } else { if(typeof Swal !== 'undefined') Swal.fire('Gagal', r.message, 'error'); }
        } catch(e) { if(typeof Swal !== 'undefined') Swal.fire('Error', 'Error koneksi', 'error'); }
    });
