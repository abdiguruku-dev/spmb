...
// ========================================================
// CORE: SUBMIT FORM PENDAFTARAN & EDIT
// ========================================================
            ...
            if(result.status === 'success') {
                if(typeof Swal !== 'undefined') {
                    Swal.fire({
                        ...
                        if (resSwal.isConfirmed) { if(tempEditId) { switchView('tabel'); } else { bukaFormBaru(); } } else { switchView('tabel'); }
                    });
                }
...
function renderTabel(data) {
    ...
    data.forEach(m => { 
        ...
        let viewBtn = `<button type="button" onclick="viewData('${safeId}')" class="text-blue-600 hover:bg-blue-50 p-1.5 rounded-lg transition-colors" title="Resume Data Lengkap"><i class="ph ph-eye text-lg"></i></button>`;
        let editBtn = isCrud ? `<button type="button" onclick="editData('${safeId}')" class="text-amber-600 hover:bg-amber-50 p-1.5 rounded-lg transition-colors" title="Edit Data"><i class="ph ph-pencil-simple text-lg"></i></button>` : '';
        let delBtn = isDewa ? `<button type="button" onclick="deleteData('${safeId}')" class="text-red-600 hover:bg-red-50 p-1.5 rounded-lg transition-colors" title="Hapus Data"><i class="ph ph-trash text-lg"></i></button>` : '';
        let actionBtn = `<div class="flex justify-center gap-1">${viewBtn}${editBtn}${delBtn}</div>`;
        if(!isCrud && !isDewa) actionBtn = `<button type="button" onclick="viewData('${safeId}')" class="text-blue-600 bg-blue-50 px-3 py-1.5 rounded-lg hover:bg-blue-100 text-xs font-bold transition-colors shadow-sm"><i class="ph ph-eye text-sm align-middle"></i> Detail</button>`;
...
function filterTabel(keyword) {
    ...
    if(keyword.startsWith('filter:')) { 
        ...
        filtered = masterData.filter(m => m[field] === val); if(infoFilter) infoFilter.innerHTML = `Menyaring berdasarkan <span class="font-bold text-blue-600">${val}</span>`; 
    } else if (keyword.startsWith('missing:')) { 
        const f = keyword.split(':')[1]; filtered = masterData.filter(m => m[f] === "Belum Ada"); if(infoFilter) infoFilter.innerHTML = `Menampilkan data murid dengan berkas <span class="text-red-600 font-semibold">Belum Lengkap</span>`; 
    } else { 
...
function doDrillDownUtama(field, value) { 
    ...
    switchView('tabel'); let sInp = document.getElementById('searchInput');
...
function renderStokLogistik() {
    ...
    let rCheck = currentUserRole ? currentUserRole.trim().toLowerCase() : ""; let allowEdit = (rCheck === 'administrator' || rCheck === 'logistik' || rCheck === 'petugas seragam');
    ...
    baseItemsLogistik.forEach((item, index) => {
        ...
        if(tbody) {
            let btnEdit = allowEdit ? `<button type="button" onclick="editStok('${item}', ${awal}, ${masuk})" class="bg-blue-600 hover:bg-blue-700 text-white px-2 py-1 rounded shadow text-xs font-bold transition-colors"><i class="ph ph-pencil-simple"></i> Edit</button>` : `<span class="text-xs text-slate-400 italic">Read-only</span>`;
            let globalKey = 'stok_belum_' + index; 
            let btnLihat = butuh > 0 ? `<button type="button" onclick="showDetailTable('Belum Dapat: ${item}', 'dashboard_belum_${index}')" class="bg-red-50 hover:bg-red-100 text-red-600 border border-red-200 px-2 py-1 rounded shadow-sm text-xs font-bold transition-colors ml-1 mt-1 xl:mt-0"><i class="ph ph-users"></i> Siapa?</button>` : '';
...
function renderLogistik(data) {
    ...
    data.forEach(m => {
        ...
        let btn = allowEdit ? `<button type="button" onclick="bukaModalLogistik('${m.id_pendaftaran}')" class="bg-blue-600 hover:bg-blue-700 text-white px-3 py-1.5 rounded shadow text-xs font-bold transition-colors">Distribusi</button>` : `<button type="button" onclick="bukaModalLogistik('${m.id_pendaftaran}')" class="text-blue-600 bg-blue-50 px-3 py-1.5 rounded-lg hover:bg-blue-100 text-xs font-bold transition-colors shadow-sm"><i class="ph ph-eye text-sm align-middle"></i> View</button>`;
...
function renderVisitAndRiwayat(data) {
    ...
    data.forEach((m) => {
        ...
        // SAFE HTML ESCAPING
        let safeNama = m.nama_lengkap ? m.nama_lengkap.replace(/"/g, '&quot;').replace(/'/g, '&#39;') : "Unknown";
        let safeTim = m.tim_alokasi ? m.tim_alokasi.replace(/"/g, '&quot;').replace(/'/g, '&#39;') : "-";
        
        let aksiBtn = `<button type="button" class="btn-lapor-visit bg-purple-600 hover:bg-purple-700 text-white px-3 py-1.5 rounded-lg text-xs font-bold whitespace-nowrap shadow-sm transition-colors" data-id="${m.id_visit}" data-nama="${safeNama}" data-tim="${safeTim}"><i class="ph ph-note-pencil mr-1 align-middle"></i>Lapor</button>`;
...
// ========================================================
// MANAJEMEN USER (ADMINISTRATOR DEWA)
// ========================================================
...
async function tampilkanApp(userData) {
    ...
    switchView('dashboard'); fetchReferences(); 
    await loadStokData(); 
